...
file_list=list.files(pattern="\\.virusclip2.final.txt",path=outdir,recursive = TRUE)
...
p=ggplot(hist_dat, aes(x=Var1,y=Freq)) +
  ...
  theme_bw()+theme(panel.background = element_blank(),panel.grid = element_blank(),panel.border = element_rect(size=1))+scale_x_discrete(breaks=seq(0,virus_len,1000))+xlab("Virus genome")+ylab("% Virus integration")
print(p)
dev.off()



file_list2=list.files(pattern="\\.insert$",path=outdir,recursive = TRUE)
file_list2=paste(outdir,file_list2,sep='/')

insert_merge_dat=lapply(file_list2,function(f) {dat=read.table(f,sep='\t',header=F,stringsAsFactors = F) 
dat=dat[,c(1,2)]
dat$sample=gsub(".virus.insert","",basename(f))
return (dat)})

insert_merge_dat <- do.call(rbind, insert_merge_dat)
colnames(insert_merge_dat)=c("site","read_count","sample")


pdf(paste(outdir,"/plot/virus_insert_histogram.pdf",sep='/'),6,3)
p=ggplot(insert_merge_dat, aes(x=site,group=sample,color=sample)) +
  geom_density()+theme_bw()+theme(panel.background = element_blank(),panel.grid = element_blank(),panel.border = element_rect(size=1))+xlab("Length of inserted virus")
print(p)
dev.off()
