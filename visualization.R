...
p=ggplot(insert_merge_dat, aes(x=site,group=sample,color=sample)) +
  geom_density()+theme_bw()+theme(panel.background = element_blank(),panel.grid = element_blank(),panel.border = element_rect(linewidth=1))+xlab("Length of inserted virus")
print(p)
dev.off()
